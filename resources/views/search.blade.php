                    ...
                    <div class="card shadow">
                        @foreach ($id as $id)
                            ...
                            <div class="card-footer">
                                <button type="submit" class="btn btn-success mb-2"><a class="text"
                                        href="{{ route('edit', $id->id) }}">Изменить</a> </button>
                                <form class='mb-2' action="{{ route('delete', $id->id) }}" method="POST">
                                    @csrf
                                    @method('DELETE')
                                    <button type="submit" class="btn btn-danger"><a
                                            class="text">Удалить</a></button>
                                </form>
                                <button type="button" class="btn btn-primary mb-2"><a class="text"
                                        href="{{ route('index') }}">Вернуться</a></button>
                            </div>
                        @endforeach
                    </div>
